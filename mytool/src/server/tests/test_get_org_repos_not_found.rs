...
#[actix_web::test]
async fn test_get_org_repos_not_found() -> Result<()> {
    let mock_gh_client = Arc::new(MockGitHubClient::new()); // No org repos added
    let org = "nonexistent_org";
    ...
    assert!(response_body.starts_with(&Bytes::from(format!("Failed to fetch org {} repositories", org))));
