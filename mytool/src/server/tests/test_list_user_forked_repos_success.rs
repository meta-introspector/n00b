...
use super::super::handlers::{
    list_user_forked_repos_handler
};

#[actix_web::test]
async fn test_list_user_forked_repos_success() -> Result<()> {
    let mock_gh_client = Arc::new(MockGitHubClient::new());
    let username = "forkuser";
    let repo1 = create_mock_repo("fork_owner1", "forked_repo_a");
    let repo2 = create_mock_repo("fork_owner2", "forked_repo_b");
    mock_gh_client.add_user_forked_repos(username.to_string(), vec![repo1.clone(), repo2.clone()]);
    ...
    let req = test::TestRequest::get().uri(&format!("/api/github/users/{}/forked_repos", username)).to_request();
    let resp = test::call_service(&app, req).await;

    assert!(resp.status().is_success());
    let response_body: Vec<RepoMetadata> = test::read_body_json(resp).await;
    assert_eq!(response_body.len(), 2);
    assert_eq!(response_body[0].name, "forked_repo_a");
    assert_eq!(response_body[1].name, "forked_repo_b");
