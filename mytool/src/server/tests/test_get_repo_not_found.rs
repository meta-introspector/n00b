...
#[actix_web::test]
async fn test_get_repo_not_found() -> Result<()> {
    let mock_gh_client = Arc::new(MockGitHubClient::new()); // No repo added
    let owner = "nonexistent";
    let repo_name = "nonexistent";
    ...
    assert!(response_body.starts_with(&Bytes::from(format!("Failed to fetch repo {}/{}: ", owner, repo_name)))); // Added &
